...
ControLeo_LiquidCrystal::ControLeo_LiquidCrystal(void) {
...
void ControLeo_LiquidCrystal::setCursor(uint8_t col, uint8_t row)
{
...
void ControLeo_LiquidCrystal::noDisplay() {
    _displaycontrol &= ~LCD_DISPLAYON;
    command(LCD_DISPLAYCONTROL | _displaycontrol);
}
void ControLeo_LiquidCrystal::display() {
    _displaycontrol |= LCD_DISPLAYON;
    command(LCD_DISPLAYCONTROL | _displaycontrol);
}
...
void ControLeo_LiquidCrystal::noCursor() {
    _displaycontrol &= ~LCD_CURSORON;
    command(LCD_DISPLAYCONTROL | _displaycontrol);
}
void ControLeo_LiquidCrystal::cursor() {
    _displaycontrol |= LCD_CURSORON;
    command(LCD_DISPLAYCONTROL | _displaycontrol);
}
...
void ControLeo_LiquidCrystal::noBlink() {
    _displaycontrol &= ~LCD_BLINKON;
    command(LCD_DISPLAYCONTROL | _displaycontrol);
}
void ControLeo_LiquidCrystal::blink() {
    _displaycontrol |= LCD_BLINKON;
    command(LCD_DISPLAYCONTROL | _displaycontrol);
}
...
void ControLeo_LiquidCrystal::scrollDisplayRight(void) {
...
void ControLeo_LiquidCrystal::rightToLeft(void) {
    _displaymode &= ~LCD_ENTRYLEFT;
    command(LCD_ENTRYMODESET | _displaymode);
}
...
void ControLeo_LiquidCrystal::autoscroll(void) {
...
void ControLeo_LiquidCrystal::noAutoscroll(void) {
    _displaymode &= ~LCD_ENTRYSHIFTINCREMENT;
    command(LCD_ENTRYMODESET | _displaymode);
}
...
inline void ControLeo_LiquidCrystal::command(uint8_t value) {
    send(value, LOW);
}

inline size_t ControLeo_LiquidCrystal::write(uint8_t value) {
    send(value, HIGH);
    return 1;
}
...
void ControLeo_LiquidCrystal::setBacklight(uint8_t status) {
    _i2c.digitalWrite(BACKLIGHT_PIN, status);
}


// Turn the backlight on or off
void ControLeo_LiquidCrystal::setBuzzer(uint8_t status) {
    _i2c.digitalWrite(BUZZER_PIN, status);
}


// Low level data pushing commands
// Write either command or data, with automatic 4/8-bit selection
void ControLeo_LiquidCrystal::send(uint8_t value, uint8_t mode) {
...
void ControLeo_LiquidCrystal::write4bits(uint8_t value) {
    uint8_t out;
    
    out = _i2c.readGPIO();
    
    // Speed up for I2C since it is sluggish
    for (int i = 0; i < 4; i++) {
